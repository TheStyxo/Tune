...
import { Manager, ManagerSearchOptions } from "./Manager";
...
export abstract class TrackUtils {
   ...
   * @param requester
   */
  static build(data: TrackData, options: ManagerSearchOptions = {}): Track {
   ...
   * @param requester
   */
  static buildUnresolved(query: string | UnresolvedQuery, options: ManagerSearchOptions = {}): UnresolvedTrack {
  ...
  static async getClosestTrack(
    ...
    const res = await TrackUtils.manager.search(query, { uuid: unresolvedTrack.uuid, requester: unresolvedTrack.requester });
