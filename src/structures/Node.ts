...
export type ErrorsOfType = Map<string, number>;
...
export class Node {
  ...
  private errors: Map<string, ErrorsOfType> = new Map();
  ...
  protected trackEnd(player: Player, track: Track, payload: TrackEndEvent): void {
    //Handle previous tracks
    if (payload.reason === "FINISHED") player.queue.previousTracks.unshift(track);
  ...
  protected queueEnd(player: Player, track: Track, payload: TrackEndEvent): void {
    if (player && track && payload) {
      player.queue.previousTracks.unshift(track);
      this.manager.emit("trackEnd", player, track, payload);
    }
  ...
  protected handleError(player: Player, track: Track | UnresolvedTrack, payload: TrackExceptionEvent | TrackStuckEvent, type: string) {
    if (!this.errors.has(type)) this.errors.set(type, new Map())

    const errorsForType = this.errors.get(type);
    const maxErrorsPer10Seconds = player.options.maxErrorsPer10Seconds || 2;
    ...
    if (errorsForType.has(player.guild.id)) {
      ...
      if (numberOfErrors + 1 >= maxErrorsPer10Seconds) {
        this.manager.emit("criticalError", player, track, payload, type);
        return true;
      }
