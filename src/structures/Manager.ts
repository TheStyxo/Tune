...
function check(options: ManagerOptions) {
  ...
  if (!(options.client instanceof Client))
    throw new TypeError(
      'Manager option "client" must be present and be a discord "Client".'
    );
...
export class Manager extends EventEmitter {
  ...
  public init(client?: Client): this {
    ...
    if (typeof client !== "undefined") this.options.client = client;

    if (!this.options.client)
      throw new Error(
        '"client" is not set. Pass it in Manager#init() or as a option in the constructor.'
      );

    if (this.options.client && !(this.options.client instanceof Client))
      throw new TypeError(
        '"client" set is not type of discord "Client"'
      );
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const result: SearchResult = {
        ...
        tracks: res.data.tracks.map((track: TrackData) =>
          TrackUtils.build(track, options)
        ),
      };
  ...
  public updateVoiceState(data: VoicePacket): void {
    ...
    if (data.t === "VOICE_SERVER_UPDATE") {
      ...
      if (data.d.user_id !== this.options.client?.user.id) return;
      state.sessionId = data.d.session_id;
      if (player.voiceChannel.id !== data.d.channel_id) {
        const channel = this.options.client?.channels.resolve(data.d.channel_id) as VoiceChannel;
        this.emit("playerMove", player, player.voiceChannel, channel);
